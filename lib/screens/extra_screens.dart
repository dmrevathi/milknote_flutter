      ...
      body: _loading
          ...
          : Column(children: [
              Expanded(child: _items.isEmpty
                ? const Center(child: Text('எந்த அறிக்கைகளும் கிடைக்கவில்லை.',
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Card(child: ListTile(
                        ...
                        onTap: () => context.go('/monthly-calc/${item['id']}'),
                      ));
                    },
                  ),
              ),
...
class EditMilkScreen extends StatefulWidget {
  ...
  final String date;
  const EditMilkScreen({super.key, required this.connectionId, required this.date});
  @override
...
class _EditMilkScreenState extends State<EditMilkScreen> {
  ...
  Future<void> _fetch() async {
    setState(() => _loading = true);
    try {
      final res = await ApiService.getMilkByDate(widget.connectionId, widget.date);
      final raw = (res['data'] as List? ?? [])
          .where((r) => r['date'] == widget.date)
          .map((r) => {
            ...Map<String, dynamic>.from(r),
            'is_active_bool': (r['is_active'] == 1 || r['is_active'] == '1'),
          })
          .toList();
  ...
  void _addRecord() {
    ...
    if (qty == null || qty <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Valid quantity required')));
      return;
    }
    setState(() {
      _records.add({'id': null, 'quantity': qty, 'is_active': 1, 'is_active_bool': true});
      _qtyCtrl.clear();
    });
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final formatted = _records.map((r) => {
        ...r,
  ...
  Widget build(BuildContext context) {
    ...
    final displayDate = parts.length == 3
        ? '${parts[2]}-${parts[1]}-${parts[0]}' : widget.date;

    return Scaffold(
      appBar: AppBar(title: Text('பால் திருத்து ($displayDate)')),
      body: _loading
          ? const Center(child: CircularProgressIndicator(color: kGreen))
          : Column(children: [
              Padding(
                ...
                child: Row(children: [
                  Expanded(child: TextField(
                      ...
                      decoration: const InputDecoration(hintText: 'Quantity (e.g. 2.5)'))),
                  const SizedBox(width: 10),
                  ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.orange),
                      onPressed: _addRecord, child: const Text('சேர்')),
                ]),
              ),
              Expanded(child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(child: SwitchListTile(
                    ...
                    activeColor: kGreen,
                    onChanged: (v) => setState(() => _records[i]['is_active_bool'] = v),
                  ));
                },
              )),
              Padding(
...
class _CowMonthlyScreenState extends State<CowMonthlyScreen> {
  ...
  Future<void> _fetchPersons() async {
    ...
    try {
      ...
      setState(() { _persons = data is List ? data : []; });
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(e.toString()), backgroundColor: kRed));
    } finally {
        ...
        Expanded(child: _records.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Card(child: ListTile(
                  title: Text(d, style: const TextStyle(fontWeight: FontWeight.w700)),
...
class _MonthlyInputScreenState extends State<MonthlyInputScreen> {
  ...
  double get _grandTotal =>
      _data.values.fold(0.0, (s, e) => s + e[0] + e[1]);
  ...
  void _showPicker(int day, int idx, bool isFrac) {
    ...
    showModalBottomSheet(context: context, builder: (_) => Padding(
      ...
      child: Wrap(spacing: 8, runSpacing: 8, children: opts.map((v) {
        final cur = isFrac
            ? (_getVal(day, idx) % 1)
        ...
        return GestureDetector(
          onTap: () {
            ...
            if (isFrac) {
              _setVal(day, idx, _getVal(day, idx).floorToDouble() + v);
            } else {
    ...
    try {
      ...
      if (auth.userId == null) {
        showDialog(context: context, builder: (_) => AlertDialog(
          ...
          content: const Text('You need an account to save monthly records.'),
          actions: [
            TextButton(
      ...
      await ApiService.saveCalcReport(auth.userId!, 'Monthly Entry', total, dataMap);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Saved! Total: $total L'),
              backgroundColor: kGreen));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(e.toString()), backgroundColor: kRed));
      ...
      body: Column(children: [
        Container(
          ...
          child: const Row(children: [
            ...
            Expanded(child: Text('Morning', textAlign: TextAlign.center,
                style: TextStyle(fontWeight: FontWeight.w700,
                    fontSize: 12, color: kGreen))),
            Expanded(child: Text('Evening', textAlign: TextAlign.center,
                style: TextStyle(fontWeight: FontWeight.w700,
                    fontSize: 12, color: kGreen))),
          ]),
        ),
          ...
          itemBuilder: (_, i) {
            ...
            return Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: _Cell(
                    label: '.${((m % 1) * 100).toInt().toString().padLeft(2, '0')}',
                    onTap: () => _showPicker(day, 0, true), light: true)),
                Expanded(child: _Cell(
                    label: e.floor().toString(),
                    onTap: () => _showPicker(day, 1, false))),
                Expanded(child: _Cell(
                    label: '.${((e % 1) * 100).toInt().toString().padLeft(2, '0')}',
                    onTap: () => _showPicker(day, 1, true), light: true)),
              ]),
            );
          },
        )),
...
class _Cell extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    onTap: onTap,
    child: Container(
      margin: const EdgeInsets.all(2), height: 40,
      decoration: BoxDecoration(
