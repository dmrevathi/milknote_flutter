...
class _MonthlyReportScreenState extends State<MonthlyReportScreen> {
  ...
  Future<void> _fetchPersons() async {
    ...
    try {
      final auth = context.read<AuthProvider>();
      // Load milk person name and phone saved during login
      final prefs = await SharedPreferences.getInstance();
      _milkPersonName = prefs.getString('milk_person_name') ?? '';
  ...
  Future<void> _exportPdf(Map t) async {
    ...
    // Translated labels for PDF
    final labelMilkPerson = t['sideBar']?['registerMilkPerson'] != null
        ? 'Milk Person' : 'Milk Person';
    final labelCowPerson  = 'Cow Person';
    final labelMonth      = t['monthlyMilkReport']?['month_label'] ?? 'Month';
    final labelGenAt      = 'Generated At';
    final labelNo         = '#';
    final labelDate       = t['dailyMilkReport']?['dateLabel'] ?? 'Date';
    final labelQty        = t['monthlyMilkReport']?['liter_label'] != null
        ? '${t['monthlyMilkReport']?['milk_label'] ?? 'Milk'} (${t['monthlyMilkReport']?['liter_label'] ?? 'L'})'
        : 'Qty (L)';
    final labelTotal      = t['monthlyMilkReport']?['total_milk_label'] ?? 'Total';
    final milkPersonDisplay = _milkPersonName.isNotEmpty
        ? '$_milkPersonName
$_milkPersonPhone'
        : '-';

    // Rebuild table rows with translated headers
    final tableRowsFinal = <pw.TableRow>[
      pw.TableRow(
        decoration: pw.BoxDecoration(color: PdfColor.fromHex('1bbd36')),
        children: [
          _pdfCell(labelNo, isHeader: true),
          _pdfCell(labelDate, isHeader: true),
          _pdfCell(labelQty, isHeader: true),
        ],
      ),
      ...tableRows.skip(1), // skip old header row
    ];
      ...
      build: (pw.Context ctx) => [
        ...
        pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
              pw.Text('Milk Note - ${t['sideBar']?['monthlyMilkRecord'] ?? 'Monthly Report'}',
                  style: pw.TextStyle(color: PdfColors.white,
                      fontSize: 20, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 4),
              pw.Text('$labelMonth: $monthStr  |  ${sorted.length} records',
        ...
        // Info grid
        pw.Row(children: [
          pw.Expanded(child: _pdfInfoBox(labelMilkPerson, milkPersonDisplay)),
          pw.SizedBox(width: 10),
          pw.Expanded(child: _pdfInfoBox(labelCowPerson, '$cowName
$cowPhone')),
        ]),
        pw.SizedBox(height: 8),
        pw.Row(children: [
          pw.Expanded(child: _pdfInfoBox(labelMonth, monthStr)),
          pw.SizedBox(width: 10),
          pw.Expanded(child: _pdfInfoBox(labelGenAt, now)),
        ]),
        ...
        pw.Container(
          ...
          child: pw.Text('$labelTotal: $_total ${t['monthlyMilkReport']?['liter_label'] ?? 'L'}',
              style: pw.TextStyle(color: PdfColors.white,
  ...
  Widget build(BuildContext context) {
    final t = context.watch<LangProvider>().t;
    final auth = context.read<AuthProvider>();
    // milkPersonName loaded from SharedPreferences in _fetchPersons
      ...
      appBar: MilkNoteAppBar(
        ...
        actions: _records.isNotEmpty ? [
          IconButton(
            ...
            tooltip: 'Export PDF',
            onPressed: () => _exportPdf(context.read<LangProvider>().t),
          ),
        ] : null,
      ),
      body: Column(children: [
